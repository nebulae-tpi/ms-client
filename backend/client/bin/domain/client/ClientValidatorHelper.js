...
class ClientValidatorHelper {
   ...
   * @param {*} roles
   */
  static checkClientCreationClientValidator$(
    client,
    authToken,
    roles,
    userMongo
  ) {
    return of({ client, authToken, roles }).pipe(
      mergeMap(data => this.checkTokenValidity$().pipe(mapTo(data))),
      tap(data => {
        if (!data.client) this.throwCustomError$(USER_MISSING_DATA_ERROR_CODE);
      }),
      tap(data => {
        if (!data.client.businessId)
          this.throwCustomError$(MISSING_BUSINESS_ERROR_CODE);
      }),
      mergeMap(data =>
        this.checkEmailExistKeycloakOrMongo$(
          data.client.generalInfo.email
        ).pipe(mapTo(data))
      )
    );
  }
   ...
   * @param {*} roles
   */
  static checkClientUpdateClientValidator$(
    client,
    authToken,
    roles,
    userMongo
  ) {
    return of({ client, authToken, roles, userMongo: userMongo }).pipe(
      mergeMap(data => this.checkTokenValidity$().pipe(mapTo(data))),
      tap(data => {
        if (!data.client) this.throwCustomError$(USER_MISSING_DATA_ERROR_CODE);
      }),
      ...
      tap(data =>
        this.checkIfUserBelongsToTheSameBusiness(
          data.userMongo,
          data.authToken,
          "Client",
          data.roles
   ...
   * @param {*} roles
   */
  static checkClientUpdateClientSatelliteValidator$(
    client,
    authToken,
    roles,
    userMongo
   ...
   * @param {*} roles
   */
  static checkClientUpdateClientStateValidator$(
    client,
    authToken,
    roles,
    userMongo
    ...
    return of({ client, authToken, roles, userMongo: userMongo }).pipe(
      ...
      tap(data =>
        this.checkIfUserIsTheSameUserLogged(data.client, authToken, "Client")
      ),
      tap(data =>
        this.checkIfUserBelongsToTheSameBusiness(
          data.userMongo,
  ...
  static checkClientValidateNewClient$() {
    return TokenKeycloakDA.checkTokenValidity$().pipe(
      catchError(error => {
        console.log(
          "An error ocurred checking keycloak token validity: ",
          error
        );
   ...
   * @param {*} roles
   */
  static checkClientCreateClientAuthValidator$(
    client,
    authToken,
    roles,
    userMongo
    ...
    return of({ client, authToken, roles, userMongo: userMongo }).pipe(
      ...
      tap(data => {
        if (
          !data.authInput ||
          !data.authInput.username.trim().match(userNameRegex)
        )
          this.throwCustomError$(INVALID_USERNAME_FORMAT_ERROR_CODE);
      }),
      tap(data => {
        if (!data.userMongo) this.throwCustomError$(USER_NOT_FOUND_ERROR_CODE);
      }),
      mergeMap(data =>
        this.checkUsernameExistKeycloak$(
          data.authInput,
          data.client.authInput.username
        ).pipe(mapTo(data))
      ),
      tap(data =>
        this.checkIfUserBelongsToTheSameBusiness(
   ...
   * @param {*} roles
   */
  static checkClientUpdateClientAuthValidator$(
    client,
    authToken,
    roles,
    userMongo
  ...
  static checkClientRemoveClientAuthValidator$(
    client,
    authToken,
    roles,
    userMongo
    ...
    return of({ client, authToken, roles, userMongo: userMongo }).pipe(
      ...
      tap(data => {
        if (!data.userMongo.auth || !data.userMongo.auth.username)
  ...
  static checkIfUserWasDeletedOnKeycloak$(userKeycloakId) {
    return of(userKeycloakId).pipe(
      mergeMap(userKeycloakId =>
        ClientKeycloakDA.getUserByUserId$(userKeycloakId)
      ),
      tap(userKeycloak => {
        if (userKeycloak) this.throwCustomError$(USER_WAS_NOT_DELETED);
      })
    );
  }
   ...
   * @param {*} roles
   */
  static checkIfUserBelongsToTheSameBusiness(
    userMongo,
    authToken,
    context,
    roles
    ...
    if (
      !userMongo ||
      (!roles["PLATFORM-ADMIN"] && userMongo.businessId != authToken.businessId)
    ) {
  ...
  static checkEmailExistKeycloakOrMongo$(email, userMongo) {
    ...
    return of(emailLowercase).pipe(
      ...
      mergeMap(([keycloakResult, mongoResult]) => {
        const userKeycloak = this.searchUserKeycloakByEmail(
          keycloakResult,
          emailLowercase
        );
        const userKeycloakId =
          userMongo && userMongo.auth && userMongo.auth.userKeycloakId
            ? userMongo.auth.userKeycloakId
            : undefined;
        if (
          userKeycloak &&
          (!userKeycloakId || userKeycloakId != userKeycloak.id)
        ) {
  ...
  static checkUsernameExistKeycloak$(user, username) {
    return ClientKeycloakDA.getUser$(username).pipe(
      mergeMap(keycloakResult => {
        const userKeycloak = this.searchUserKeycloakByUsername(
          keycloakResult,
          username
        );
        if (userKeycloak) {
  ...
  static searchUserKeycloakByUsername(keycloakResult, username) {
    if (keycloakResult && keycloakResult.length > 0) {
      return keycloakResult.find(
        userKeycloak =>
          userKeycloak.username.toLowerCase() == username.toLowerCase()
      );
    }
    return null;
  }
  ...
  static checkUserEmailExistKeycloak$(user, email) {
    return ClientKeycloakDA.getUser$(null, email).pipe(
      mergeMap(userFound => {
  ...
  static throwCustomError$(errorCode) {
    return throwError(
      new CustomError(context, "Client", errorCode.code, errorCode.description)
    );
  }
}
